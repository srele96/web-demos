...
const SECTIONS = [
  {
    ...
    pages: [
      { eyebrow: "01", title: "One card, one screen.",
  ...
  },
  {
    ...
    pages: [
      { eyebrow: "02", title: "Its own physics.",
  ...
  },
  {
    ...
    pages: [
      { eyebrow: "03", title: "When content won't fit —",
      ...
      { eyebrow: "03", title: "So it's paged instead.",
      ...
      { eyebrow: "03", title: "Nested, not stacked.",
  ...
  },
  {
    ...
    pages: [
      { eyebrow: "04", title: "Three ways to move.",
...
function SectionCard({ section, cardRef, reveal }) {
  ...
  return (
    <div className="slide-card" ref={cardRef}>
      <div className="slide-bg" style={{ backgroundImage: `url(${section.img})` }} />
      ...
      {multi ? (
        <>
          <Swiper
            ...
            pagination={{ el: ".inner-pagination-" + section.id, clickable: true }}
          ...
          <div className={"swiper-pagination inner-pagination inner-pagination-" + section.id} />
...
function App() {
  ...
  const reveal = useCallback((el, delay = 0) => {
    ...
    gsap.fromTo(
      ...
      { y: 24, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.5, delay, stagger: 0.08, ease: "power2.out" }
    );
  }, []);
  ...
  useEffect(() => {
    cardRefs.current.forEach((el, i) => {
      if (el && i !== 0) {
        gsap.set(el.querySelectorAll(".card-title, .card-body"), { opacity: 0, y: 24 });
      }
    });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!rootEl) {
    console.error('No element with id="root" found — check src/index.html markup.');
    return;
  }
